...
--[[
TODO: Write simple assembler for this kind of test
...
local function load_file(filename, offset)
	local offset = tonumber(offset) or 0
	local f = assert(io.open(filename, "r"), "Can't open file!")
	local str = f:read("*a")
	for i=1, #str do
		mem:write_8(i-1+offset, str:byte(i))
	end
end
...
if false then
	...
	load_file("hello_world.bin")
end
cpu:run()
-- register x1 should be 0x00 00 00 FF (ADDI  0 0x00F -> x1, ADDI x1, 0x0F0 -> x1)
